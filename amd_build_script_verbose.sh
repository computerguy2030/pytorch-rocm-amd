...
if lsb_release -d | grep Ubuntu; then
    ...
    if ! uname -r | grep 5.4; then
        ...
        if lsb_release -d | grep 20.10; then 
            ...
            wget -nc https://kernel.ubuntu.com/~kernel-ppa/mainline/v5.4.42/linux-headers-5.4.42-050442_5.4.42-050442.202005200734_all.deb 
            wget -nc https://kernel.ubuntu.com/~kernel-ppa/mainline/v5.4.42/linux-headers-5.4.42-050442-generic_5.4.42-050442.202005200734_amd64.deb 
            wget -nc https://kernel.ubuntu.com/~kernel-ppa/mainline/v5.4.42/linux-image-unsigned-5.4.42-050442-generic_5.4.42-050442.202005200734_amd64.deb 
            wget -nc https://kernel.ubuntu.com/~kernel-ppa/mainline/v5.4.42/linux-modules-5.4.42-050442-generic_5.4.42-050442.202005200734_amd64.deb 
            sudo dpkg -i *.deb || exit
        ...
        if { lsb_release -d | grep 18.04 || lsb_release -d | grep 20.04; }; then
            sudo apt install linux-image-5.4.0-42-generic -y || exit; 
    ...
    rm -r ~/Downloads/5.4_kernel 
    sudo apt --purge autoremove -y $(dpkg --list | grep 'linux-image\|linux-modules\|linux-headers' | grep -v 'linux-.*5.4.*' | cut -d " " -f 3) || exit
    sudo apt update
    fi
...
echo ''
if sudo pip3 install dist/*.whl; then
    echo ''
